...
| `depends_on` | `MVP-001`, `BUS-MVP-001`, `REL-PLAY-001`, `UX-MVP-001`, `UI-MVP-001`, `ART-MVP-001`, `TECH-SIM-001`, `TECH-DATA-001` |
...
- `ART-MVP-001`의 캐주얼 의사 3D는 3/4 직교로 미리 렌더한 2D PNG다. MVP 런타임에 `Node3D`, 3D 카메라·조명·물리 재질을 추가하지 않는다.
- 타이틀 배경과 시설·아이템 PNG는 종횡비를 보존하고 linear filtering으로 표시한다. 배경은 cover, 투명 전경 에셋은 contain을 사용하며 텍스처 크기가 탭 영역과 레이아웃을 결정하지 않는다.
...
data/
  ...
  rounds.json
art/mvp/runtime/
  backgrounds/  facilities/  items/  # ART-MVP-001의 오프라인 2D PNG
...
- 현재 1차 아트 패스는 `art/mvp/runtime/`의 타이틀 배경 1장, 시설 `384×384` RGBA PNG 7장, 아이템 `256×256` RGBA PNG 8장이다. 정확한 파일 전수와 카메라·광원 규칙은 `ART-MVP-001` §2~§3이 소유한다.
- 표현 계층은 콘텐츠 ID를 파일명 또는 전용 표현 catalog에 매핑한다. `sim/`, 저장 DTO와 `data/*.json`에 텍스처 경로, 표시 크기나 색을 넣지 않는다.
- 화면이 사용하는 PNG는 고정된 표현 catalog·preload·Godot Resource 중 하나로 참조하고 APK·AAB 포함 검사를 통과시킨다. 실행 중 디렉터리 스캔과 원격 다운로드는 금지한다. 개발 중 누락 에셋은 기존 텍스트 UI로 안전하게 대체할 수 있지만 같은 누락은 사전 검증·출시 후보를 차단한다.
- painted sprite는 CanvasItem linear filtering을 사용한다. 현재 개별 PNG를 유지하며, `ART-MVP-001` §7.3의 스타일·가독성·메모리 검증 전에는 아틀라스를 성급히 만들지 않는다.
- 장식용 `TextureRect`는 입력을 가로채지 않는다. 기존 `UI-MVP-001`의 44×44 탭 영역, 고정 3×2 시설 위치, 큰 글씨와 색 보조 상태가 텍스처보다 우선한다.
- 저메모리 상황에서 캐시를 비워도 패키지 리소스를 다시 읽을 수 있어야 하며, 아트 로드 성공 여부가 시뮬레이션 상태·명령 순서·저장 해시를 바꾸지 않는다.
...
모든 게임 데이터, 한국어 카피, 폰트, `ART-MVP-001` 런타임 에셋, 개인정보처리방침과 오픈소스 고지는 AAB 안에 포함한다. 인터넷이 전혀 없고 Play 서비스가 비활성인 기기에서도 신규 시작부터 R5 정산까지 동작해야 한다.
...
7. `ART-MVP-001`의 참조된 타이틀·시설·아이템 에셋 포함과 로드 확인
8. debug APK 신규 설치와 업데이트 설치
9. 비행기 모드 cold start와 snapshot 복구
...
- API 24와 물리 기기에서 `ART-MVP-001`의 타이틀 배경·시설·아이템이 누락·검은 알파 halo·nearest 계단 현상 없이 linear filtering으로 표시되는지 확인한다.
- 360×640과 긴 화면에서 painted sprite가 종횡비를 유지하고 44×44 탭 영역, 큰 글씨, 세이프 에어리어와 겹치지 않는지 확인한다.
...
- 참조된 `art/mvp/runtime/` 에셋이 APK·AAB에 오프라인으로 포함되고 그 표시 설정이 `ART-MVP-001`과 일치한다.
