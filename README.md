...
Android 전용 무료 MVP를 위한 Godot 4.7 프로젝트다. 현재 구현은 최종 아트 이전에 전체 흐름과 핵심 재미를 검증하는 **오프라인 그레이박스 플레이테스트 빌드**다.
...
에디터에서 프로젝트를 실행하면 세로형 그레이박스 UI가 열린다. 새 게임을 시작해 R1부터 공급→변환→제작→납품을 직접 조작하고, 별 1개 이상으로 완료하면 다음 라운드로 진행한다.
...
- 개발에 참여하지 않은 첫 플레이어 5명의 무설명 그레이박스 테스트와 관찰·인터뷰
- 테스트 결과에 따른 코어 UX·밸런스 수정과 재검증
- 그 이후의 최종 아트·스토어 이미지, Dungeon Office 전용 upload key, Play Console 등록
